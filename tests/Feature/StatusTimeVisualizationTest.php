...
test('task modal does NOT show status time section when task has no status changes', function () {
    ...
    $task = Activity::withoutEvents(fn () => Activity::factory()->create(['status' => ActivityStatus::Inbox]));
...
test('dashboard only considers tasks completed in last 30 days', function () {
    ...
    $oldTask = Activity::withoutEvents(fn () => Activity::factory()->done()->create([
        'completed_at' => now()->subDays(60),
    ]));

    ActivityStatusChange::create([
        'activity_id' => $oldTask->id,
        'from_status' => null,
        'to_status' => ActivityStatus::Inbox,
...
test('get-task status_timeline includes duration_minutes for each change', function () {
    $task = Activity::withoutEvents(fn () => Activity::factory()->create(['status' => ActivityStatus::Todo]));
