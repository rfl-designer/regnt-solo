...
- **Partida a frio ancora no corte e ignora o limiar**: sem nenhuma conclusão Intangível no histórico, o relógio conta do `BaselineCut` vigente (`anchor = cut`) e o alerta **dispara independentemente do limiar**, inclusive num corte feito hoje — "nunca concluí um Intangível" é a fome máxima, não ausência de dado, e o limiar só serve para envelhecer uma conclusão de verdade. Os `days` continuam vindo do corte (dizem desde quando o quadro está sendo observado), mas não decidem nada; abaixo de um dia o texto vira "feito hoje". Sem corte nenhum, ancora na primeira mudança de status já registrada (`board`, mesma regra); um quadro sem histórico algum não tem relógio e fica calado (`none`)
...
- **Tabela `policy_versions` append-only**, no espírito do `ActivityStatusChange`: `key`, `body` (markdown), `note` opcional ("por que mudou"), `created_at`. Vigente = **última linha por chave**, ordenada por `id` e não por `created_at` — duas versões salvas no mesmo segundo (um typo corrigido em seguida) ainda têm ordem inequívoca, e "vigente" não pode ser cara ou coroa. `PolicyVersion::record()` é a única escrita suportada e **sempre insere**, e o append-only é **imposto por recusa, não por convenção**: `updating`/`deleting` no model barram a instância carregada e um `PolicyVersionBuilder` barra `update`/`delete`/`upsert` em massa (que nunca passam pelos eventos). O valor da tabela é o texto anterior continuar legível — um único `update()` esquecido destruiria exatamente isso. A recusa cobre o Eloquent, que é por onde a aplicação escreve; um `DB::table()` cru ainda passaria, e nada aqui toma esse caminho. A nota é opcional porque a v1 não tem versão anterior a explicar, e uma nota obrigatória só seria preenchida com ruído — nota em branco vira `null`
- **O editor não pré-preenche a nota**: ela explica *esta* mudança, não a anterior
- **O corpo é renderizado por `App\Support\Markdown::render()`**, não por `Str::markdown()`: é texto do usuário, e o CommonMark padrão deixaria passar HTML cru e link `javascript:` num painel aberto na sessão autenticada. As três contagens de versão são agregadas no banco (`count(*)` + `group by`) — hidratar um log append-only inteiro para produzir três números cresceria sem teto com o uso. As ações `edit`/`toggleHistory`/`save` usam `tryFrom` e recusam em silêncio: `#[Locked]` protege propriedade hidratada, não argumento de ação
