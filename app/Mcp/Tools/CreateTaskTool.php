...
class CreateTaskTool extends Tool
{
    ...
    protected string $description = 'Creates a personal task (type=Task). Personal tasks are local operational/personal to-dos with no GitHub mirror; they never appear on the stakeholder board. Project and parent are optional: a task may stand alone, or hang off an Epic or Issue via parent_id. A task may also be linked directly to a client via client_id when it has no project — setting both project_id and client_id keeps only the project link. Defaults to status inbox and priority medium. When status is "done", the task is marked done (completed_at set, running timers stopped).';
    ...
    public function handle(Request $request): Response
    {
        $validated = $request->validate([
            ...
            'client_id.exists' => 'Client not found. Use list-projects or the clients page to find available client ids.',
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'client_id' => $schema->integer()->description('Id of the client to link the task to directly. Optional, and only meaningful when project_id is not set — setting both keeps only the project link.'),
