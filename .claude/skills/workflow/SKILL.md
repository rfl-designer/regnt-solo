...
Antes de tudo, verifique se está na branch `main` ou `develop`, se não estiver, mude para a ela e execute o `git pull origin *`

## Fases do Workflow

### Fase 1: Iniciar Trabalho

Utilize o agente `brain` para planejar o trabalho:

1. **Buscar detalhes do Projeto** - `mcp__soloboard__get-project`
2. **Buscar detalhes da task** - `mcp__soloboard__get-task`
3. **Iniciar timer** - `mcp__soloboard__start-timer task_id=X focus=true`

Utilize o agente `git-specialist` para planejar a branch: 4. **Criar branch** - `git checkout -b feat/nome-da-feature`

### Fase 2: Implementação

Utilize o agente `implementer` para implementar:

1. **Implementar** - Código seguindo convenções

Utilize o agente `git-specialist` para commitar: 2. **Commits incrementais** - Conventional commits

Utilize o agente `tester` para testar: 3. **Testar** - `php artisan test --compact --filter=X`
...
- "usar o agente implementer para fazer mudanças"
...
Utilize o laravel-simplifier para revisar o que foi implementado: 3. **Revisar código** - Verificar padrões

### Fase 4: Finalização

**IMPORTANTE:** O timer só para APÓS criar a PR.

Utilize o agente `git-specialist` para concluir o workflow git:
